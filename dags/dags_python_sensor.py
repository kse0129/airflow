...
with DAG(
    ...
    def check_api_update(http_conn_id, endpoint, base_dt_col, **kwargs):
        ...
        try:
            ...
            AirflowException(f'{base_dt_col} 컬럼은 YYYY.MM.DD 혹은 YYYY/MM/DD 형태가 아닙니다')

        today_ymd = kwargs.get('data_interval_end').in_timezone('Asia/Seoul').strftime('%Y-%m-%d')
        if last_date >= today_ymd:
            print(f'갱신 확인(배치일 : {today_ymd} / API 갱신일 : {last_date})')
            return True
        else:
            print(f'갱신 미완료(배치일 : {today_ymd} / API 갱신일 : {last_date})')
            return False
